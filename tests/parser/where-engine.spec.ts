...
describe('QueryWhereEngine', () => {
  ...
  describe('explicit operators', () => {
    const scalarOperators: [string, Record<string, unknown>, FilterExpression][] = [
      [
        'eq',
        { status: { eq: 'ACTIVE' } },
        { field: 'status', operator: 'eq', value: 'ACTIVE', caseSensitive: false },
      ...
      [
        'neq',
        { status: { neq: 'ACTIVE' } },
        { field: 'status', operator: 'neq', value: 'ACTIVE', caseSensitive: false },
      ],
      [
        'gt',
        { age: { gt: 18 } },
      ...
      [
        ...
        { field: 'firstName', operator: 'contains', value: 'abebe', caseSensitive: false },
      ],
      [
        'startsWith',
        { firstName: { startsWith: 'abe' } },
        { field: 'firstName', operator: 'startsWith', value: 'abe', caseSensitive: false },
      ],
      [
        'isNull',
        { email: { isNull: true } },
    ...
    const arrayOperators: [string, Record<string, unknown>, FilterExpression][] = [
      [
        'in (array)',
  ...
  describe('type parsing', () => {
    const typeCases: [string, Record<string, unknown>, unknown][] = [
      ...
      ['number', { age: 25 }, 25],
      ['date', { createdAt: '2026-01-15T10:30:00.000Z' }, '2026-01-15T10:30:00.000Z'],
    ];
    ...
    it('rejects empty number values', () => {
      expect(() => QueryWhereEngine.buildFilters({ age: '' }, TEST_SPEC)).toThrow('Invalid number');
      expect(() => QueryWhereEngine.buildFilters({ age: '   ' }, TEST_SPEC)).toThrow(
        'Invalid number',
      );
    });
  ...
  describe('error handling', () => {
    const errorCases: [string, Record<string, unknown>, string][] = [
      ['unknown field', { unknown: 'value' }, "Unknown filter field: 'unknown'"],
      ['unsupported operator', { age: { unsupported: 18 } }, 'not supported'],
      ['invalid boolean value', { isActive: 'invalid' }, 'Invalid boolean value'],
    ...
    for (const [label, input, message] of errorCases) {
      it(`throws on ${label}`, () => {
        expect(() => QueryWhereEngine.buildFilters(input, TEST_SPEC)).toThrow(message);
      });
    }
  });
  ...
  describe('relation filters', () => {
    ...
    it('throws on relation with non-object value', () => {
      expect(() =>
        QueryWhereEngine.buildFilters({ member: 'invalid' }, SPEC_WITH_RELATION),
    ...
    it('handles nested relation filters', () => {
      ...
      expect(result.relations).toEqual([
        {
          relation: 'member',
          filters: [{ field: 'firstName', operator: 'eq', value: 'Abebe', caseSensitive: false }],
        },
      ]);
    });
    ...
    it('preserves the full relation path for nested relations', () => {
      ...
      const result = QueryWhereEngine.buildFilters({ org: { parent: { name: 'x' } } }, spec);
      expect(result.relations).toEqual([
        {
          relation: 'org.parent',
          filters: [{ field: 'name', operator: 'eq', value: 'x', caseSensitive: false }],
        },
      ]);
    });
  });
  ...
  describe('nesting depth', () => {
    ...
    it('allows nesting within limit', () => {
      ...
      const result = QueryWhereEngine.buildFilters({ level1: { name: 'test' } }, specWithinLimit);
  ...
  describe('value constraints', () => {
    ...
    const constraintErrors: [string, Record<string, unknown>, string][] = [
      ...
      ['string longer than maxLength', { name: 'a'.repeat(51) }, 'at most 50 characters'],
      ...
      ['non-integer when isInteger required', { score: 3.5 }, 'must be an integer'],
    ];

    for (const [label, input, message] of constraintErrors) {
      it(`throws on ${label}`, () => {
        expect(() => QueryWhereEngine.buildFilters(input, CONSTRAINED_SPEC)).toThrow(message);
      });
    }
    ...
    it('uses custom error messages', () => {
      expect(() => QueryWhereEngine.buildFilters({ shortCode: 'ab' }, CONSTRAINED_SPEC)).toThrow(
        'Too short',
      );
      expect(() =>
        QueryWhereEngine.buildFilters({ shortCode: 'abcdef' }, CONSTRAINED_SPEC),
      ).toThrow('Too long');
    });
    ...
    it('does not apply whole-value constraints to substring operators', () => {
      const spec = defineQuery({
        fields: {
          email: q.string().email().operators(q.op.contains().startsWith().endsWith()),
      ...
      const result = QueryWhereEngine.buildFilters(
        { email: { contains: 'foo' }, shortName: { startsWith: 'ab' }, code: { endsWith: 'z1' } },
        spec,
      );
  ...
  describe('max filter count', () => {
    ...
    it('throws when filter count exceeds max', () => {
      expect(() => QueryWhereEngine.buildFilters({ a: '1', b: '2' }, makeSpec(), 1)).toThrow(
        'Too many filters',
      );
    });

    it('passes when filter count is within max', () => {
      const result = QueryWhereEngine.buildFilters({ a: '1', b: '2' }, makeSpec(), 10);
