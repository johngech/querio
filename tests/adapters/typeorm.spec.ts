...
describe("TypeORM Adapter", () => {
  ...
  describe("buildWhere", () => {
    ...
    for (const [label, queryOverrides, expected] of filterCases) {
      it(`builds ${label}`, () => {
        expect(
          typeormQueryAdapter.buildWhere(makeQuery(queryOverrides)),
        ).toEqual(expected);
      });
    }
            ...
            relations: [
              {
                relation: "member",
                filters: [
                  { field: "firstName", operator: "eq", value: "Abebe" },
                ],
              },
            ],
          }),
    ...
    it("builds search as OR-array of where alternatives", () => {
      ...
      expect(result).toEqual([
        { firstName: ILike("%abebe%") },
        { lastName: ILike("%abebe%") },
      ]);
    });
  ...
  describe("buildSkipTake", () => {
    ...
    for (const [page, limit, expected] of skipTakeCases) {
      it(`calculates skip/take for page=${page}, limit=${limit}`, () => {
        expect(typeormQueryAdapter.buildSkipTake(page, limit)).toEqual(
          expected,
        );
      });
    }
