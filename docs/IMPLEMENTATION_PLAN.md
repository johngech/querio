...
| Errors | Single `QuerioError` + `ErrorCode` enum, `statusCode: 400`, structured `field`/`operator`/`path`/`details` |
...
| Build | Bun workspaces, dual ESM/CJS core output, per-package `tsconfig.build.json`, adapters typecheck against core dist |
